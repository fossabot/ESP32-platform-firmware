...
// mutex for accessing driver_disobey_samd_state, driver_disobey_samd_handlers,
// etc..
  ...
  while (1) {
    if (xSemaphoreTake(driver_disobey_samd_intr_trigger, portMAX_DELAY)) {
      ...
      for (uint8_t i = 0; i < 6; i++) {
        if (((state >> i) & 0x01) && (!((old_state >> i) & 0x01)))
          pressed |= (1 << i);
        if (((old_state >> i) & 0x01) && (!((state >> i) & 0x01)))
          released |= (1 << i);
      }
...
void driver_disobey_samd_intr_handler(void *arg) { /* in interrupt handler */
