#ifndef DRIVER_EINK_LUT_H
#define DRIVER_EINK_LUT_H

#include <stdint.h>

/** the needed size of the buffer used in driver_eink_generate_lut() */
#define DRIVER_EINK_LUT_MAX_SIZE 70
...
extern const struct driver_eink_lut_entry driver_eink_lut_full[];
/** screen update which just sets the black and white */
extern const struct driver_eink_lut_entry driver_eink_lut_normal[];
/** same as driver_eink_lut_normal but with shorter timings */
extern const struct driver_eink_lut_entry driver_eink_lut_faster[];
/** same as driver_eink_lut_faster but with shorter timings */
extern const struct driver_eink_lut_entry driver_eink_lut_fastest[];
