...
static void driver_eink_set_ram_area(uint8_t x_start,
  ...
  driver_eink_dev_write_command_p4(0x45, y_start & 0xff, y_start >> 8,
                                   y_end & 0xff, y_end >> 8);
}
...
static void driver_eink_write_bitplane(const uint32_t *buf) {
  ...
  driver_eink_dev_write_command_stream_u32(0x24, buf,
                                           DISP_SIZE_X_B * DISP_SIZE_Y / 4);
}

void driver_eink_update(const uint32_t *buf,
  ...
  driver_eink_dev_write_command_p2(0x0f, upd_conf->y_start & 0xff,
                                   upd_conf->y_start >> 8);
...
void driver_eink_display_greyscale(const uint8_t *img,
                                   driver_eink_flags_t flags,
                                   int layers) {
  // start with black.
  ...
  for (int layer = 0; layer < layers; layer++) {
    ...
    if (driver_eink_dev_type == DRIVER_EINK_DEPG0290B1 &&
        driver_eink_have_oldbuf == false && p == 1 && t > 1 &&
        layer + 1 < layers) {
...
esp_err_t driver_eink_init(void) {
  ...
  // allocate buffers
  driver_eink_tmpbuf =
      heap_caps_malloc(DISP_SIZE_X_B * DISP_SIZE_Y, MALLOC_CAP_32BIT);
  ...
  if (driver_eink_dev_type == DRIVER_EINK_GDEH029A1) {
    ...
    driver_eink_dev_write_command_p1(
        0x2c, 0xa8);  // 2C: write VCOM register (VCOM 7c)
    driver_eink_dev_write_command_p1(
        0x11, 0x03);  // 11: data entry mode setting (X inc, Y inc)
  }
  ...
  if (driver_eink_dev_type == DRIVER_EINK_DEPG0290B1) {
    ...
    driver_eink_dev_write_command_p3(
        0x01, 0x27, 0x01, 0x00);  // Set display size and driver output control
    driver_eink_dev_write_command_p1(
        0x11, 0x03);  // Ram data entry mode (Adress counter is updated in Y
                      // direction, Y increment, X increment)
    driver_eink_dev_write_command_p2(0x44, 0x00,
                                     0x0F);  // Set RAM X address (00h to 0Fh)
    driver_eink_dev_write_command_p4(
        0x45, 0x00, 0x00, 0x27, 0x01);  // Set RAM Y address (0127h to 0000h)
    driver_eink_dev_write_command_p1(
        0x3C, 0x01);  // Set border waveform for VBD (see datasheet)
    driver_eink_dev_write_command_p1(0x2C,
                                     0x26);  // Set VCOM value (SET VOLTAGE)
    driver_eink_dev_write_command_p1(
        0x03, 0x17);  // Gate voltage setting (17h = 20 Volt, ranges from 10v to
                      // 21v) (SET VOLTAGE)
    driver_eink_dev_write_command_p3(
        0x04, 0x41, 0x00, 0x32);  // Source voltage setting (15volt, 0 volt and
                                  // -15 volt) (SET VOLTAGE)
  }
