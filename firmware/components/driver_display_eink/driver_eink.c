...
static const uint8_t xlat_curve[256] = {
    ...
    0x20, 0x21, 0x22, 0x22, 0x23, 0x23, 0x24, 0x25, 0x25, 0x26, 0x26, 0x27, 0x27, 0x28, 0x29, 0x29,
    ...
    ,
};
...
void driver_eink_update(const uint32_t *buf, const struct driver_eink_update *upd_conf) {
  ...
  if (driver_eink_dev_type == DRIVER_EINK_DEPG0290B1 && driver_eink_have_oldbuf)
    driver_eink_dev_write_command_stream_u32(0x26, driver_eink_oldbuf,
                                             DISP_SIZE_X_B * DISP_SIZE_Y / 4);
...
void driver_eink_display_greyscale(const uint8_t *img, driver_eink_flags_t flags, int layers) {
  ...
  for (int layer = 0; layer < layers; layer++) {
    ...
    if (driver_eink_dev_type == DRIVER_EINK_DEPG0290B1 && driver_eink_have_oldbuf == false &&
        p == 1 && t > 1 && layer + 1 < layers) {
    ...
    for (int j = 0; j < p; j++) {
      ...
      memset_u32(&buf[(y_end + 1) * DISP_SIZE_X_B / 4], 0,
                 (DISP_SIZE_Y - y_end - 1) * DISP_SIZE_X_B / 4);
...
esp_err_t driver_eink_init(void) {
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "nvs handle failure");
    return err;
  }
  ...
  if (driver_eink_tmpbuf == NULL) {
    ESP_LOGE(TAG, "tmpbuf alloc no mem");
  ...
  if (driver_eink_dev_type == DRIVER_EINK_DEPG0290B1) {
    ...
    if (driver_eink_oldbuf == NULL) {
      ESP_LOGE(TAG, "oldbuf alloc no mem");
      return ESP_ERR_NO_MEM;
    }
  }
  ...
  if (driver_eink_dev_type == DRIVER_EINK_GDEH029A1) {
    ...
    driver_eink_dev_write_command_p3(0x0c, 0xd7, 0xd6,
                                     0x9d);        // 0C: booster soft start control
  ...
  if (driver_eink_dev_type == DRIVER_EINK_DEPG0290B1) {
    ...
    driver_eink_dev_write_command_p3(0x01, 0x27, 0x01,
                                     0x00);        // Set display size and driver output control
    driver_eink_dev_write_command_p1(0x11, 0x03);  // Ram data entry mode (Adress counter is updated
                                                   // in Y direction, Y increment, X increment)
    driver_eink_dev_write_command_p2(0x44, 0x00,
                                     0x0F);  // Set RAM X address (00h to 0Fh)
    driver_eink_dev_write_command_p4(0x45, 0x00, 0x00, 0x27,
                                     0x01);        // Set RAM Y address (0127h to 0000h)
    driver_eink_dev_write_command_p1(0x3C, 0x01);  // Set border waveform for VBD (see datasheet)
    driver_eink_dev_write_command_p1(0x2C,
                                     0x26);        // Set VCOM value (SET VOLTAGE)
    driver_eink_dev_write_command_p1(0x03, 0x17);  // Gate voltage setting (17h = 20 Volt, ranges
                                                   // from 10v to 21v) (SET VOLTAGE)
    driver_eink_dev_write_command_p3(0x04, 0x41, 0x00, 0x32);  // Source voltage setting (15volt, 0
                                                               // volt and -15 volt) (SET VOLTAGE)
  }
