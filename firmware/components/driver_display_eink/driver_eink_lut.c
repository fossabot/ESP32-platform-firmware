...
int driver_eink_lut_generate_gdeh029a1(const struct driver_eink_lut_entry *list,
  ...
  int pos = 0;
  while (list->length != 0) {
    uint8_t voltages = driver_eink_lut_conv(list->voltages, flags);
    int len          = list->length;
    while (len > 0) {
...
#ifdef CONFIG_DRIVER_EINK_LUT_DEBUG
  {
    ESP_LOGD(TAG, "dump.");
    char line[10 * 3 + 1];
    char *lptr = line;
...
#endif  // CONFIG_DRIVER_EINK_LUT_DEBUG
...
int driver_eink_lut_generate(const struct driver_eink_lut_entry *list,
  ...
  if (driver_eink_dev_type == DRIVER_EINK_DEPG0290B1) {
    return driver_eink_lut_generate_depg0290b1(list, flags, lut);
  }
