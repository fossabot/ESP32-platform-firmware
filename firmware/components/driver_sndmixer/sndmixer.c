...
// Grabs a new ID by atomically increasing curr_id and returning its value. This
// is called outside of the audio playing thread, hence the atomicity.
static uint32_t new_id() {
  uint32_t old_id, new_id;
  do {
    old_id = curr_id;
    new_id = old_id + 1;
    // compares curr_id with old_id, sets to new_id if same, returns old val in
    // new_id
...
static int init_source(int ch,
                       ...
                       const void *data_end) {
  int chunksz = srcfns->init_source(data_start, data_end, samplerate,
                                    &channel[ch].src_ctx);
  if (chunksz <= 0)
    return 0;  // failed
  ...
  channel[ch].dds_acc  = chunksz
                        << 16;  // to force the main thread to get new data
  return 1;
}
...
static void handle_cmd(sndmixer_cmd_t *cmd) {
  if (cmd->cmd == CMD_QUEUE_WAV || cmd->cmd == CMD_QUEUE_MOD) {
    ...
    if (cmd->cmd == CMD_QUEUE_WAV) {
      r = init_source(ch, &sndmixer_source_wav, cmd->queue_file_start,
                      cmd->queue_file_end);
    } else if (cmd->cmd == CMD_QUEUE_MOD) {
      r = init_source(ch, &sndmixer_source_mod, cmd->queue_file_start,
                      cmd->queue_file_end);
    }
    if (!r) {
    ...
    // Rest are all commands that act on a certain ID. Look up if we have a
    // channel with that ID first.
    int ch = -1;
    for (int x = 0; x < no_channels; x++) {
...
static void sndmixer_task(void *arg) {
  ...
  while (1) {
    ...
    for (int i = 0; i < CHUNK_SIZE; i++) {
      int s = 0;  // current sample value, multiplied by 256 (because of
                  // multiplies by channel volume)
      for (int ch = 0; ch < no_channels; ch++) {
        if (channel[ch].source) {
          ...
          if ((channel[ch].dds_acc >> 16) >= channel[ch].chunksz) {
            // That value is outside the channels chunk buffer. Refill that
            // first.
            int r = channel[ch].source->fill_buffer(channel[ch].src_ctx,
                                                    channel[ch].buffer);
            if (r == 0) {
              // Source is done.
            ...
            channel[ch].dds_acc -=
                (channel[ch].chunksz
                 << 16);  // reset dds acc; we have parsed chunksize samples.
            channel[ch].chunksz = r;  // save new chunksize
          }
          // Multiply by volume, add to cumulative sample
          s += channel[ch].buffer[channel[ch].dds_acc >> 16] *
               channel[ch].volume;
        }
      }
...
int sndmixer_init(int p_no_channels) {
  ...
  int r = xTaskCreatePinnedToCore(&sndmixer_task, "sndmixer", 2048, NULL, 5,
                                  NULL, MY_CORE);
  if (!r) {
    free(channel);
...
// The following functions all are essentially wrappers for the axt of pushing a
