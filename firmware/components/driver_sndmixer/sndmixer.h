...
typedef struct {
   ...
   * wrong. Returns amount of bytes in buffer (normally what init_source
   * returned) otherwise. */
 ...
 *
 * @note This function internally calls kchal_sound_start, there is no need to
 * do this in your program if you use this function to initialize the sound
 * mixer.
 *
 ...
 *
 * This queues a sound to be played. It will not be actually played until
 * sndmixer_play is called.
 *
 * @param wav_start Start of the wav-file data
 * @param wav_end End of the wav-file data
 * @param evictable If true, if all audio channels are filled and a new sound is
 ...
 *
 * This queues a piece of tracked music to be played. It will not be actually
 * played until sndmixer_play is called.
 *
 ...
 *
 * @warning This may not be implemented yet. TODO: remove this warning when
 * implemented.
 *
 ...
 *
 * A queued sound will always start off with a volume setting of 255 (max
 * volume). This call can be used to adjust the volume of the sound at any time
 * afterwards.
 *
 * @param id ID of the sound, obtained when queueing it
 ...
 * @brief Play a sound
 *
 * When a sound is queued, it is not playing yet. Use this call to start
 * playback. You can also use this call to resume a sound paused by
 * sndmixer_pause or sndmixer_pause_all.
 *
 * @param id ID of the sound, obtained when queueing it
 */
 ...
 *
 * This can be used when e.g. the game is paused. Sounds can be individually
 * un-paused afterwards and new sounds can still be queued and played, given
 * enough free/evictable channels.
 */
