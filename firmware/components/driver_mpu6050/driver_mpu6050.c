...
esp_err_t driver_mpu6050_configure_dlpf(uint8_t dlpf) {
  ...
  if (res != ESP_OK)
    return res;
  res = driver_i2c_write_reg(CONFIG_I2C_ADDR_MPU6050, MPU6050_CONFIG,
...
esp_err_t driver_mpu6050_configure_gyro_range(uint8_t fs_sel) {
  ...
  if (res != ESP_OK)
    return res;
  res = driver_i2c_write_reg(CONFIG_I2C_ADDR_MPU6050, MPU6050_GYRO_CONFIG,
                             (fs_sel & 0x03)
...
esp_err_t driver_mpu6050_configure_accel_range(uint8_t afs_sel) {
  ...
  if (res != ESP_OK)
    return res;
  res = driver_i2c_write_reg(CONFIG_I2C_ADDR_MPU6050, MPU6050_ACCEL_CONFIG,
                             (afs_sel & 0x03)
                                 << 3);  // Set full scale range, ignore self-test features
  return res;
}
...
static inline esp_err_t _init(void) {
  if (driver_i2c_write_reg(CONFIG_I2C_ADDR_MPU6050, MPU6050_SMPRT_DIV, 0x00) != ESP_OK)
    return ESP_FAIL;
  if (driver_i2c_write_reg(CONFIG_I2C_ADDR_MPU6050, MPU6050_CONFIG, 0x00) != ESP_OK)
    return ESP_FAIL;
  if (driver_i2c_write_reg(CONFIG_I2C_ADDR_MPU6050, MPU6050_GYRO_CONFIG, 0x08) != ESP_OK)
    return ESP_FAIL;
  if (driver_i2c_write_reg(CONFIG_I2C_ADDR_MPU6050, MPU6050_ACCEL_CONFIG, 0x00) != ESP_OK)
    return ESP_FAIL;
  if (driver_i2c_write_reg(CONFIG_I2C_ADDR_MPU6050, MPU6050_PWR_MGMT_1, 0x01) != ESP_OK)
    return ESP_FAIL;
  return ESP_OK;
}
