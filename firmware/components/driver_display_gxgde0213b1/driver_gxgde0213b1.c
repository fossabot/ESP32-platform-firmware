...
static esp_err_t driver_gxgde0213b1_release_spi(void) {
...
static esp_err_t driver_gxgde0213b1_claim_spi(void) {
  if (spi_bus != NULL)
    return ESP_OK;  // already claimed?
  ESP_LOGI(TAG, "claiming VSPI bus");
  driver_vspi_release_and_claim(driver_gxgde0213b1_release_spi);
  ...
  static const spi_bus_config_t buscfg = {
      ...
      .quadwp_io_num   = -1,
      .quadhd_io_num   = -1,
      .max_transfer_sz = GXGDE0213B1_MAX_TRANSFERSIZE,
  };
...
esp_err_t driver_gxgde0213b1_init(void) {
  ...
  if (driver_gxgde0213b1_init_done)
    return ESP_OK;
...
esp_err_t driver_gxgde0213b1_write(const uint8_t *buffer) {
  ...
  for (uint8_t i = 0; i < 8; i++) {
    res = driver_gxgde0213b1_send(buffer + (GXGDE0213B1_MAX_TRANSFERSIZE * i),
                                  GXGDE0213B1_MAX_TRANSFERSIZE, true);
  }
  if (res != ESP_OK)
    return res;
