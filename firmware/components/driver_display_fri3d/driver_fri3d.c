...
esp_err_t driver_fri3d_send_line(uint8_t x, uint8_t brightnessPos) {
  ...
  if (x < 7) {
    if (internalBuffer) {
      for (uint8_t i = 0; i < 7; i++) {
        if (internalBuffer[(FRI3D_WIDTH * (4 - i)) + x + 0] >
            (BRIGHTNESS_STEPS - brightnessPos) * (256 / BRIGHTNESS_STEPS))
          a |= (1 << i);
        if (internalBuffer[(FRI3D_WIDTH * (4 - i)) + x + 7] >
            (BRIGHTNESS_STEPS - brightnessPos) * (256 / BRIGHTNESS_STEPS))
          b |= (1 << i);
      }
    }
  ...
  spi_transaction_t t = {.length = 24, .tx_buffer = buffer};
