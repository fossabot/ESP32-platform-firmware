...
void driver_mpr121_intr_task(void *arg) {
  ...
  while (1) {
    if (xSemaphoreTake(driver_mpr121_intr_trigger, portMAX_DELAY)) {
      ...
      // if (old_gpio_state != gpio_state) printf("MPR121 GPIO STATE CHANGED %d
...
esp_err_t driver_mpr121_configure(const uint32_t *baseline,
  ...
  for (int i = 0; i < 12; i++) {
    if (driver_mpr121_is_touch_input(i)) {
      ...
      if (res != ESP_OK)
        return res;
      res = driver_mpr121_write_reg(MPR121_RELEASETH_0 + 2 * i,
                                    release);  // release
      if (res != ESP_OK)
        return res;
      if (baseline != NULL) {
        // printf("MPR121 baseline #%u is now set to %u >> 2 = %u\n", i,
        // baseline[i], baseline[i] >> 2);
        res = driver_mpr121_write_reg(MPR121_BASELINE_0 + i,
                                      baseline[i] >> 2);  // baseline
        if (res != ESP_OK)
          return res;
      }
    }
