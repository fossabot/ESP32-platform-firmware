...
// SDMMC HOST peripheral pin mapping (note: these are bound by the chip and can
// NOT be changed!)
...
esp_err_t driver_sdcard_mount(const char *mount_point, bool format_if_mount_failed) {
  ...
  driver_mpr121_set_gpio_level(CONFIG_DRIVER_SDCARD_MPR121_PIN,
                               true);  // Enable power
#endif
