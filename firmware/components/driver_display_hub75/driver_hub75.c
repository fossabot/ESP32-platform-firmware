...
// reproduce an 16-bit image more or less faithfully, though. Currently set to 6
// to improve scrolling text.
...
int backbuf_id = 0;  // which buffer is the backbuffer, as in, which one is not
                     // active so we can write to it
...
void render16() {
  for (unsigned int y = 0; y < 8; y++) {
    int lbits = 0;  // Precalculate line bits of the *previous* line, which is
                    // the one we're displaying now
    if ((y - 1) & 1)
      lbits |= BIT_A;
    if ((y - 1) & 2)
      lbits |= BIT_B;
...
// Warning when set too high: Do not look into LEDs with remaining eye.
  ...
  for (int i = 0; i < ((1 << BITPLANE_CNT) - 1); i++) {
    ...
    // Magic to make sure we choose this bitplane an appropriate time later next
    // time
