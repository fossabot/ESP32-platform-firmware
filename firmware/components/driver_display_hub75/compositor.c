 ...
 * x, y is the coordinate of the top left corner of the text block. each
...
void renderText(char *text, Color color, int x, int y, int sizeX, int skip, bool firstshow) {
  ...
  for (int i = 0; i < strlen(text); i++) {
    ...
    if (skip == 0)
      x++;  // If started printing insert blank line
    else
      skip--;  // If not decrease the number to skip by one to make it fluid
  }
}
