...
int16_t dirty_x1 = FB_WIDTH - 1,
        dirty_y1 = FB_HEIGHT - 1;  // Bottom-right corner of the "dirty" area
...
void driver_framebuffer_set_dirty_area(int16_t x0,
  ...
  if (dirty_x0 > FB_WIDTH - 1)
    dirty_x0 = FB_WIDTH - 1;
  if (dirty_y0 > FB_HEIGHT - 1)
    dirty_y0 = FB_HEIGHT - 1;
  if (dirty_x1 > FB_WIDTH - 1)
    dirty_x1 = FB_WIDTH - 1;
  if (dirty_y1 > FB_HEIGHT - 1)
