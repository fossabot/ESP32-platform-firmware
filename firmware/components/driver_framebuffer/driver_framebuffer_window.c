...
/* Private functions */
inline Window *_create_window() {  // Create an empty window object
...
inline void _add_window(Window *window) {  // Add a window to the linked list
  if (windows == NULL) {
    windows = window;
    return;
  }
...
inline void _remove_window_from_linked_list(
    Window *window) {  // Remove a window from the linked list
  Window *prevWindow = window->_prevWindow;
  Window *nextWindow = window->_nextWindow;
  if (prevWindow)
    prevWindow->_nextWindow = nextWindow;  // Link previous to next
  if (nextWindow)
    nextWindow->_prevWindow = prevWindow;  // Link next to previous
  if (windows == window)
    windows = nextWindow;  // Change the start window of the linked list
}
