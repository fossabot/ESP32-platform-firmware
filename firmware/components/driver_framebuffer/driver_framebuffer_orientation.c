...
bool driver_framebuffer_orientation_apply(Window *window,
  ...
  if (*orientation == reverse_portrait)
    orientationString = "reverse_portrait";
  if (*orientation == landscape)
    orientationString = "landscape";
  if (*orientation == reverse_landscape)
    orientationString = "reverse_landscape";

  // if (window) printf("Orientation: %s = %s  %d=>%d  %d=>%d\n", window ?
  // window->name : "global", orientationString, oldx, *x, oldy, *y);
